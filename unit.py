...
class SendersTests(unittest.TestCase):
    ...
    @mock.patch('senders.SMS.twilio.rest.Client')
    def test_twilio(self, mock_twilio):
        ...
        twilio.messages.create.return_value=message()
        sender.send()
        self.assertTrue(twilio.messages.create.called)
        twilio.messages.create.assert_called_once_with(body=sender.msg, to=sender.intlcode+sender.a['addr'], from_=sender.desde['addr'])
        self.assertEqual(str(type(sender.message.sid)),"<class 'str'>")
        self.assertRegex(sender.message.sid, r"^SM[\w]{32}$")

        twilio.messages.create.side_effect=SMS.TwilioRestException("404","Not Found")
